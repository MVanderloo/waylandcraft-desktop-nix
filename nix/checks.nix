...
in
{
  ...
  shell-lint =
    pkgs.runCommand "waylandcraft-shell-lint-check"
      ...
      ''
        shellcheck \
          ${../scripts/client-probe} \
          ${../scripts/demo-prepare} \
          ${../scripts/diagnose} \
          ${../scripts/offline-uuid} \
          ${../scripts/runtime-prepare} \
          ${../scripts/tty-cage-smoke} \
          ${../scripts/waylandcraft-demo}
        touch "$out"
      '';
  ...
  documentation = pkgs.runCommand "waylandcraft-documentation-check" { } ''
    ...
    grep -Fq '[configuration fragment](examples/configuration.nix)' \
      ${../README.md}
    grep -Fq '[manual hardware checklist](docs/manual-testing.md)' \
      ${../README.md}
    grep -Fq 'weekend hobby project' ${../README.md}
    grep -Fq 'does not guarantee a response or fix' ${../CONTRIBUTING.md}
    grep -Fqx '# Manual hardware checklist' ${../docs/manual-testing.md}
    grep -Fqx '## Test notes' ${../docs/manual-testing.md}
    test ! -e ${../.}/SECURITY.md
    if grep -RniE 'security policy|reporting a vulnerability|security fixes are made' \
      ${../README.md} \
      ${../CONTRIBUTING.md} \
      ${../docs} \
      ${../.github/ISSUE_TEMPLATE}; then
      echo 'documentation still advertises a security-response policy' >&2
      exit 1
    fi
  ...
  offline-resolution =
    pkgs.runCommand "waylandcraft-offline-resolution-check"
      ...
      ''
        ...
        test ! -e ${packages.minecraftHome}/bin
