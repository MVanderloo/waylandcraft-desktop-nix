...
Include the locked revision, system closure, GPU and driver, display manager,
launch path, probe results, and the first relevant journal error. State which
target-machine checks in [the development guide](development.md#target-machine-check)
were run.
